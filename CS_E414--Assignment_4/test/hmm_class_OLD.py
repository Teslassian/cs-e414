...
class HMM(object):
    ...
    def _viterbi(signal, trans, dists):
        ...
        # Initialize delta_m and back_p matrices 

        delta_m = np.zeros(shape=(n_states, n_obs))
        back_p = np.zeros(shape=(n_states, n_obs), dtype=int) #dtype is int because values are used as indices

        back_p.T[0] = -1 # Set all values of the first column to -1  
        

        # Calculate delta_0

        for j in range(n_states):                
            delta_m[j,0] = dists[j].logf(signal.T[0]) + np.log(trans[-1, j])
        ...
        for t in range(1, n_obs):
            ...
            for j in range(n_states):
                ...
                for i in range(n_states):
                    ...
                    A_delta.append(np.log(trans[i,j]) + delta_m[i, t-1])

                A_delta = np.array(A_delta)
                A_delta_max = np.max(A_delta)

                delta_m[j,t] = dists[j].logf(signal.T[t]) + A_delta_max
                back_p[j, t] = np.argmax(A_delta)        

        # Calculate delta_T

        A_delta = []
        for j in range(n_states):
            A_delta.append(np.log(trans[j, -1]) + delta_m[j, n_obs-1])
        ...
        seq = np.zeros(shape=(n_obs,), dtype=int)

        seq[0] = np.argmax(A_delta)

        for i in range(1, n_obs):
            seq[i] = back_p[seq[i-1], n_obs-i]
    ...
    def _forward(signal, trans, dists):
        ...
        alpha_m = np.zeros(shape=(n_states, n_obs))                

        # Calculate alpha_0

        for j in range(n_states):                
            alpha_m[j,0] = dists[j].logf(signal.T[0]) + np.log(trans[-1, j])

        # Calculate alpha_t

        for t in range(1, n_obs):
            for j in range(n_states):
                p_x = dists[j].logf(signal.T[t])               

                temp_list = []

                for i in range(n_states):  
                    temp_list.append(np.log(trans[i,j]) + alpha_m[i, t-1])                    
                ...
                sum = temp_max + np.log(sum)

                alpha_m[j,t] = p_x + sum

                if np.isnan(alpha_m[j,t]):
                    alpha_m[j,t] = np.NINF
        ...
        for j in range (0, n_states):            
            temp_list.append(np.log(trans[j, -1]) + alpha_m[j, n_obs-1])

        temp = np.array(temp_list)
        temp_max = np.max(temp)      
           
        for j in range(n_states):
            ll += np.exp(temp[j] - temp_max)

        ll = temp_max + np.log(ll)
    ...
    def _calcstates(self, trans, dists):
        ...
        n_states = trans.shape[1]-1

        ll = 0
        states = np.zeros(shape=(n_obs, n_states), dtype=bool)

        index = 0

        # Run viterbi on all signals

        for signal in self.signals:
            seq, loglikelihood = self._viterbi(signal, trans, dists) 

            # Add all loglikelihoods
            ll += loglikelihood           

            # Loop through the returned sequence and build the states array
            for i, s in enumerate(seq):
                states[index + i, s] = True

            index += seq.shape[0] # Index used to keep track of last position changed in the states array
        
        #TODO Remove unused states

        # Initialize trans matrix
        trans = np.zeros(shape=(n_states+1, n_states+1))

        # The function given in the book -- basically just count all transitions from i to j
        index = 0
        for signal in self.signals:            
            ...
            for i in range(n_states):                
                ...
                # Count transitions from null entry state to i
                if states[index, i] == True:
                    trans[-1, i] += 1

                # Count transitions from i to null exit state
                if states[index + s_len - 1, i] == True:
                    trans[i, -1] += 1

                # Count transitions from i to j
                for j in range(n_states):
                    for t in range(index, index + s_len - 1):
                        if states[t, i] == True and states[t+1,j] == True:                        
                            trans[i,j] += 1           
                
            index += s_len

        # Normalize all rows to get probability
    ...
    def _updatecovs(self, states):
        ...
        for state in states.T:
            f_data = self.data.T[state]

            cov = np.cov(f_data, rowvar=False, bias=True)
            mean = np.mean(f_data)         
        ...
        covs = np.array(covs_list).reshape(n_states, d, d)
        means = np.array(means_list).reshape(n_states, d)
    ...
    def _em(self, trans, states):
        ...
        while not converged and iters <  self.maxiters:            
            ...
            covs, means = self._updatecovs(oldstates) # Initialize the covariances and means using the initial state allocation                       
            dists = [Gaussian(mean=means[i], cov=covs[i]) for i in range(len(covs))]
            oldstates, trans, newLL = self._calcstates(trans, dists)            

            diff = abs(oldLL - newLL)
